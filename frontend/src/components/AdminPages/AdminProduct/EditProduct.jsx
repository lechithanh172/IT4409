...
// --- Dữ liệu cứng Category/Brand ---
const allCategory = [
  { categoryId: 1, name: "Laptop" },
  { categoryId: 2, name: "Tablet" },
  { categoryId: 3, name: "Smartphone" },
  { categoryId: 4, name: "Accessory" },
  { categoryId: 5, name: "Monitor" },
  { categoryId: 6, name: "Printer" },
  { categoryId: 7, name: "Router" },
  { categoryId: 8, name: "Speaker" },
  { categoryId: 9, name: "Camera" },
  { categoryId: 10, name: "Smartwatch" },
  { categoryId: 13, name: "bàn phím" },
  { categoryId: 14, name: "chuột" },
...
const EditProduct = ({ product, setModalChild, handleRefresh }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      form.setFieldsValue({
        ...
        brandName: product.brandName, // Đặt giá trị cho brandName ở đây
      });

      // Tìm và đặt logo ban đầu (logic này vẫn đúng để hiển thị ảnh)
      const brandData = brandOptions.find(
        (b) => b.value === product.brandName
      );
      // console.log("Brand data found for image: ", brandData); // Có thể giữ lại để debug nếu cần
      setSelectedBrandImage(brandData ? brandData.image : null);
  ...
  }, [product, form]); // Bỏ initialBrand khỏi dependencies nếu có
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      message.success(`(Giả lập) Sản phẩm ${data.productName} được cập nhật!`); // Thay bằng gọi API thật
      // --- Kết thúc gọi API ---
      ...
      console.error("Update Product Error:", e.response || e);
      ...
      >
        <Row gutter={24}>
          <Col xs={24} md={12}>
            ...
            <Form.Item
              label="Thương Hiệu"
              name="brandName" // Ant Form sẽ quản lý giá trị của Select này
              ...
              <Row gutter={16} align="middle">
                <Col flex="auto">
                  <Select
                    ...
                    // defaultValue={initialBrand.label} // ---- BỎ DÒNG NÀY ----
                    onChange={handleBrandChange} // Vẫn giữ để cập nhật ảnh khi thay đổi
                    style={{ width: "100%" }}
                  />
                </Col>
                <Col>
        ...
        <Divider style={{ flexShrink: 0, margin: '0 0' }} />
      {/* **** TĂNG PADDING DỌC Ở ĐÂY **** */}
      <Form.Item style={{ textAlign: "right", marginBottom: 0, padding: '10px 10px', flexShrink: 0 }}> {/* Tăng từ 16px lên 24px (hoặc giá trị khác nếu muốn) */}
        <Space>
           {/* Có thể tăng size nút nếu muốn */}
