    ...
    <header className={styles.header}>
      <div className={styles.container}>
            ...
            {isCategoryDropdownOpen && (
              <div className={`${styles.dropdownMenu} ${styles.categoryDropdown}`}>
                {categories.map((category) => (
                  <Link to={`/products?category=${category.name}`} className={styles.dropdownItem} key={category.categoryId} onClick={toggleCategoryDropdown}>
                    {category.name}
                  </Link>
                ))}
        ...
        <div className={styles.actions}>
          ...
          {isMenu || isSearch ? <div ref={overlayRef} className={styles.overlay} onClick={closeOverlay}></div> : null}
          ...
          <div className={styles.desktopAuth}>
            {isLoggedIn ? (
              ...
              <>
                <Button variant="secondary" size="small" onClick={() => setIsLoggedIn(true)} className={styles.authButton}>
                  Đăng nhập
                </Button>
                <Button variant="primary" size="small" onClick={() => {}} className={styles.authButton}>
                  Đăng ký
                </Button>
              </>
            )}
      ...
      <nav ref={mobileMenuRef} className={`${styles.mobileMenu} ${isMobileMenuOpen ? styles.mobileMenuOpen : ''}`}>
        ...
        <div className={styles.mobileUserInfo}>
           {isLoggedIn ? (
             <>
                ...
                <Link to="/profile" className={styles.mobileNavLink} onClick={toggleMobileMenu}>Hồ sơ</Link>
                <Link to="/orders" className={styles.mobileNavLink} onClick={toggleMobileMenu}>Đơn hàng</Link>
                <button onClick={handleLogout} className={`${styles.mobileNavLink} ${styles.mobileLogoutButton}`}>Đăng xuất</button>
             </>
           ) : (
             <div className={styles.mobileAuthButtons}>
                 <Button variant="primary" onClick={() => { setIsLoggedIn(true); toggleMobileMenu(); }} className={styles.mobileAuthBtn}>Đăng nhập</Button>
                 <Button variant="secondary" onClick={() => { toggleMobileMenu(); }} className={styles.mobileAuthBtn}>Đăng ký</Button>
             </div>
           )}
        </div>
        ...
        <div className={styles.mobileCategorySection}>
            ...
            {categories.map((category) => (
              <Link to={`/products?category=${category.name}`} className={styles.mobileNavLink} key={category.categoryId} onClick={toggleMobileMenu}>
                {category.name}
              </Link>
            ))}
        </div>
